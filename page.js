...
async function observeYouTubeNavigation() {
  if (desktop) {
    function onYtNavigateFinish(e) {
      ...
      // Default (Featured): EgC4AQCSAwDyBgQKAjIA
      // Featured: EghmZWF0dXJlZPIGBAoCMgA%3D
      // let browseEndpointParams = e.detail?.endpoint?.browseEndpoint?.params
      if (pageType == 'channel') {
        document.documentElement.setAttribute('cpfyt-channel-tab', location.pathname.split('/').at(-1) || 'featured')
        tweakChannelPage()
      } else {
    ...
    if (document.querySelector('ytd-browse[page-subtype="channels"][role="main"]')) {
      document.documentElement.setAttribute('cpfyt-channel-tab', location.pathname.split('/').at(-1) || 'featured')
      tweakChannelPage()
    } else {
      document.documentElement.removeAttribute('cpfyt-channel-tab')
